...
D = D2; % Valeurs propres de Sigma
W = (X_centre')*W2; % Vecteurs propres de Sigma (n-1 colonnes)

% Tri par ordre decroissant des valeurs propres de Sigma :
[D_triee, indices] = sort(diag(D),'descend');
D_triee = diag(D_triee);
...
for i = 1:n-1
    W_triee(:,i) = W_triee(:,i)/norm(W_triee(:,i));
end
...
for k = 1:n-1
	img = reshape(W_triee(:,k), nb_lignes,nb_colonnes);
...
% Calcul de l'individu moyen :
n = size(X_masque,1);
individu_moyen = 1/n*(X_masque')*ones(n,1);

% Centrage de la matrice X :
X_masque_centre = X_masque - ones(n,1)*(individu_moyen');

% Calcul de la matrice de covariance (impossible a calculer ainsi a cause de sa taille) :
%Sigma = transpose(X_centre)*X_centre/n;

% Calcul de la matrice resultant du calcul commuté (voir annexe du sujet) :
Sigma2 = X_masque_centre*(X_masque_centre')/n;
...
% Les vecteurs propres de Sigma (les eigenfaces) se deduisent de ceux de Sigma2 :
D = D2; % Valeurs propres de Sigma
W = (X_masque_centre')*W2; % Vecteurs propres de Sigma (n-1 colonnes)
